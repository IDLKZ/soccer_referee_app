<div class="container mx-auto px-6 py-8">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Управление сезонами</h1>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Управление футбольными сезонами и их статусами</p>
        </div>
        @if($canCreate)
                    ...
                    <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 dark:hover:from-blue-900/20 dark:hover:to-indigo-900/30 transition-all duration-150 border-l-4 border-transparent hover:border-blue-400 dark:hover:border-blue-500">
                        <td class="px-4 py-4">
                            <div class="flex items-start space-x-3">
                                <div class="flex-shrink-0 h-10 w-10">
                                    <div class="h-10 w-10 rounded-lg bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center border-2 border-white dark:border-gray-700 shadow-sm">
                                        <span class="text-white text-sm font-bold">
                                            {{ strtoupper(substr($season->title_ru, 0, 1)) }}
                                        </span>
                                    </div>
                                </div>
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400">
                                        {{ $season->title_kk }} / {{ $season->title_en }}
                                    </div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-4">
                            <div class="space-y-1">
                                <div class="flex items-center text-sm text-gray-900 dark:text-gray-100">
                                    <i class="fas fa-play-circle mr-2 text-green-500 text-xs"></i>
                                    {{ $season->start_at->format('d.m.Y') }}
                                </div>
                                <div class="flex items-center text-xs text-gray-500 dark:text-gray-400">
                                    <i class="fas fa-stop-circle mr-2 text-red-500 text-xs"></i>
                                    {{ $season->end_at->format('d.m.Y') }}
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-4 whitespace-nowrap text-center">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                @endif
                                @if($canEdit)
                                <button wire:click="toggleSeasonStatus({{ $season->id }})"
                                        class="inline-flex items-center justify-center w-8 h-8 rounded-lg {{ $season->is_active ? 'bg-yellow-50 hover:bg-yellow-100 text-yellow-600 hover:text-yellow-800 dark:bg-yellow-900/20 dark:hover:bg-yellow-900/30 dark:text-yellow-400 dark:hover:text-yellow-300' : 'bg-green-50 hover:bg-green-100 text-green-600 hover:text-green-800 dark:bg-green-900/20 dark:hover:bg-green-900/30 dark:text-green-400 dark:hover:text-green-300' }} transition-colors duration-150"
                                        title="{{ $season->is_active ? 'Деактивировать' : 'Активировать' }}">
                                    <i class="fas fa-{{ $season->is_active ? 'ban' : 'check' }} text-sm"></i>
                                </button>
                                @endif
                                @if($canDelete)
                                <button wire:click="deleteSeason({{ $season->id }})"
                                        ...
                                        title="Удалить"
                                        onclick="return confirm('Вы уверены, что хотите удалить этот сезон?')">
    ...
    <div class="text-center py-12">
        <div class="flex flex-col items-center">
            ...
            <p class="text-gray-500 dark:text-gray-400 font-medium">Сезоны не найдены</p>
    ...
    <div wire:ignore.self class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border border-gray-200 dark:border-gray-700">
                <form wire:submit.prevent="createSeason">
                    <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4 max-h-[90vh] overflow-y-auto">
                        <div class="mb-4">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">Создание нового сезона</h3>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Код сезона*</label>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="date" wire:model="startAt" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition-colors">
                                ...
                                <div>
                                    ...
                                    <input type="date" wire:model="endAt" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition-colors">
    ...
    <!-- Модальное окно редактирования сезона -->
    @if($showEditModal)
    <div wire:ignore.self class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border border-gray-200 dark:border-gray-700">
                <form wire:submit.prevent="updateSeason">
                    <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4 max-h-[90vh] overflow-y-auto">
                        <div class="mb-4">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">Редактирование сезона</h3>
                        </div>
