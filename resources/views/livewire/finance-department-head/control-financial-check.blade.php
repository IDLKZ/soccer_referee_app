    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100 flex items-center">
                <i class="fas fa-clipboard-check mr-3 text-indigo-600 dark:text-indigo-400"></i>
                Контрольная финансовая проверка АВР
            </h1>
            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                Финальное утверждение актов выполненных работ руководителем финансового департамента
            </p>
        </div>
        ...
        <div class="space-y-4">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4">
                АВР, ожидающие контрольной финансовой проверки
            </h2>
            ...
            @if($pendingAvrs->count() > 0)
                @foreach($pendingAvrs as $avr)
                    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition">
                        ...
                        @if($avr->first_financial_comment)
                            <div class="mb-4 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
                                <div class="text-xs text-green-600 dark:text-green-400 mb-1 font-medium">
                                    <i class="fas fa-comment mr-1"></i>Комментарий специалиста финансового отдела:
                                </div>
                                <div class="text-sm text-gray-900 dark:text-gray-100">
                                    {{ $avr->first_financial_comment }}
                                </div>
                            </div>
                        @endif
            ...
            @else
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-12 text-center border border-gray-200 dark:border-gray-700">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 text-lg">
                        Нет АВР, ожидающих контрольной финансовой проверки
                    </p>
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full">
                    ...
                    <div class="px-6 py-4 max-h-[70vh] overflow-y-auto">
                        ...
                        @if($selectedAvr->judge_comment || $selectedAvr->control_comment || $selectedAvr->first_financial_comment)
                            <div class="mb-4">
                                ...
                                @if($selectedAvr->control_comment)
                                    <div class="p-3 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded mb-2">
                                ...
                                @endif
                                @if($selectedAvr->first_financial_comment)
                                    <div class="p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded">
                                        <p class="text-xs text-green-600 dark:text-green-400 font-medium mb-1">Специалист финансового отдела:</p>
                                        <p class="text-sm text-gray-900 dark:text-gray-100">{{ $selectedAvr->first_financial_comment }}</p>
                                    </div>
                                @endif
                            </div>
                        @endif
                    ...
                    <div class="px-6 py-4">
                        ...
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                Комментарий руководителя {{ $confirmationAction === 'reject' ? '(обязательно)' : '(опционально)' }}
                            </label>
                            <textarea wire:model="last_financial_comment" rows="4" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 dark:bg-gray-700 dark:text-gray-100" placeholder="{{ $confirmationAction === 'reject' ? 'Укажите причину отказа...' : 'Введите комментарий (опционально)...' }}"></textarea>
                            @error('last_financial_comment')
                        ...
                        <div class="p-3 {{ $confirmationAction === 'accept' ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800' }} border rounded-lg">
                            <p class="text-sm {{ $confirmationAction === 'accept' ? 'text-green-800 dark:text-green-400' : 'text-red-800 dark:text-red-400' }}">
                                @if($confirmationAction === 'accept')
                                    После утверждения АВР будет отправлен и подтвержден. Ожидание оплаты.
                                @else
