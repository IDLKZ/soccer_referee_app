...
#[Title('Первичная проверка командировок')]
class PrimaryBusinessTripConfirmation extends Component
{
    public $selectedTrip = null;
    public $showDetailModal = false;
    public $first_comment = '';
    ...
    public function openDetailModal($tripId)
    {
        ...
        $this->first_comment = $this->selectedTrip->first_comment ?? '';
    }
    ...
    public function acceptTrip()
    {
        ...
        $finalConfirmationOperation = Operation::where('value', 'final_business_trip_confirmation')->firstOrFail();

        $this->selectedTrip->update([
            'first_status' => 1, // accepted
            'first_comment' => $this->first_comment,
            'operation_id' => $finalConfirmationOperation->id,
        ]);

        session()->flash('message', 'Командировка принята и отправлена на финальное подтверждение');
    ...
    public function rejectTrip()
    {
        ...
        $this->validate([
            'first_comment' => 'required|string|min:10',
        ], [
            'first_comment.required' => 'Укажите причину отказа',
            'first_comment.min' => 'Причина должна содержать минимум 10 символов',
        ]);

        $reprocessingOperation = Operation::where('value', 'business_trip_plan_reprocessing')->firstOrFail();

        $this->selectedTrip->update([
            'first_status' => -1, // rejected
            'first_comment' => $this->first_comment,
    ...
    public function render()
    {
        $query = Trip::with([
            ...
            ->where('first_status', 0); // waiting for first confirmation
        ...
        return view('livewire.primary-business-trip-confirmation', [
            'trips' => $trips,
            'leagues' => $leagues,
